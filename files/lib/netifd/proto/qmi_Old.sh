...
proto_qmi_setup() {
	...
	while timeout -t 5 uqmi -s -d "$device" --get-pin-status | grep '"UIM uninitialized"' > /dev/null; do
		[ -e "$device" ] || return 1
	...
	timeout -t 5 uqmi -s -d "$device" --stop-network 0xffffffff --autoconnect > /dev/null 2>&1
    timeout -t 5 uqmi -s -d "$device" --set-ip-family ipv6 --stop-network 0xffffffff --autoconnect > /dev/null 2>&1

	# Go online
	timeout -t 5 uqmi -s -d "$device" --set-device-operating-mode online > /dev/null 2>&1
	...
	timeout -t 5 uqmi -s -d "$device" --network-register > /dev/null 2>&1
