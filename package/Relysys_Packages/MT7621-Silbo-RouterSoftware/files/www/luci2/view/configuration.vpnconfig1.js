L.ui.view.extend({
  ...
  <span class="slider round"></span>`);
        ...
        l2tpConfigSectionAdd: function () {
                ...
                this.RS485GetUCISections("L2TP", "service", l2tpConfigSection).then(function (rv) {
                        var keys = Object.keys(rv);
                        var keysLength = keys.length;
                        var serverCount = 0;
                        ...
                        if (keysLength >= maxConnections) {
                                ...
                                self.RS485CreateUCISection("L2TP", "service", l2tpConfigSectionName, l2tpConfigSection).then(function (rv) {
                                        if (rv && rv.section) {
        ...
        l2tpConfigSectionRemove: function (ev) {
                ...
                self.RS485DeleteUCISection("L2TP", "service", l2tpConfigSectionName).then(function (rv) {
                        if (rv == 0) {
                                self.RS485CommitUCISection("L2TP").then(function (res) {
                                        if (res != 0) {
                                                alert("Error: Delete Configuration");
        ...
        execute: function () {
                ...
                // });
                var s = m.section(L.cbi.NamedSection, 'general', {
                        caption: L.tr('<b>NOTE: - </b>For any changes in this page to take effect, please press update button on respective VPN TAB for immediate effect. Else , reboot the board after making changes.')
                });
                ...
                self.RS485GetUCISections("L2TP", "service").then(function (rv) {
                ...
                l2tpenablebutton = function (n) {
                        ...
                        self.RS485CreateUCISection("L2TP", "service", failName, failsensorSectionOptions).then(function (rv) {
                                if (rv) {
                                        if (rv.section) {
                                                self.RS485CommitUCISection("L2TP").then(function (res) {
                                                        if (res != 0) {
