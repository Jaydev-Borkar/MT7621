L.ui.view.extend({
	...
	pfCreateFormCallback: function()
	{
		...
		s.option(L.cbi.InputValue, 'dest_ip', {
			caption:     L.tr('Destination IP Address[Optional] '),
			description: L.tr('Only match incoming traffic with this Destination IP or range.'),
			datatype:    'ip4addr',
			optional: true, 	
		...
		s.option(L.cbi.InputValue, 'dest_port', {
			...
			placeholder: "any",
		    optional: true,
			description: L.tr('Only match incoming traffic originating with the given destination port or port range on the client host'),
		});
