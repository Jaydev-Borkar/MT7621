...
proto_464xlat_setup() {
	...
	#above line is commented because 464xlat configuration should be taken from network file
	tundev=$(uci get network.$cfg.tunlink)
	...
	if [ $cfg = "wan6c1_xlat" ] 
	then
		metric=$(uci get mwan3.cwan6_1_member.metric) 
	elif [ $cfg = "wan6c2_xlat" ]
	then 
		metric=$(uci get mwan3.cwan6_2_member.metric)
	fi
