...
local specialized_interfaces = {
  modemmanager = function(_, interface)
    ...
    local info = {}
    local general_file = io.popen('cat /tmp/ModemManager_Modem_Json.txt')
    ...
    local signal_file =
      io.popen('cat /tmp/ModemManager_Signal_get.txt')
...
function interfaces.get_interface_info(name, netjson_interface)
  ...
  for _, interface in pairs(interface_data['interface']) do
    if interface['l3_device'] == name then
      ...
      local RESULT = string.match(interface['interface'], "CWAN1_0" )
      if ( RESULT == nil)  then
      else
        proto="modemmanager"
        local specialized_info = specialized_interfaces[proto] 
        if specialized_info then                                      
          info.specialized = specialized_info(name, interface)
        end
      end
      
       local RESULT = string.match(interface['interface'], "wan6c1" )
      if ( RESULT == nil)  then
      else
        proto="modemmanager"
        local specialized_info = specialized_interfaces[proto] 
       ...
       local RESULT = string.match(interface['interface'], "wan6c2" )
      if ( RESULT == nil)  then
      else
        proto="modemmanager"
        local specialized_info = specialized_interfaces[proto] 
        if specialized_info then                                      
         ...
         local RESULT = string.match(interface['interface'], "wan6c1_xlat" )
      if ( RESULT == nil)  then
      else
        proto="modemmanager"
        local specialized_info = specialized_interfaces[proto] 
        ...
        local RESULT = string.match(interface['interface'], "wan6c2_xlat" )
      if ( RESULT == nil)  then
      else
        proto="modemmanager"
        local specialized_info = specialized_interfaces[proto] 
      ...
      local RESULT = string.match(interface['interface'], "CWAN1_1" )                                                                          
      if ( RESULT == nil)  then                                                                                                                
      else                                                                                                                                     
        proto="modemmanager"                                                                                                                   
        local specialized_info = specialized_interfaces[proto]                                                                                 
      ...
      local RESULT = string.match(interface['interface'], "CWAN1" )                                                                          
      if ( RESULT == nil)  then                                                                                                                
      else                                                                                                                                     
        proto="modemmanager"                                                                                                                   
        local specialized_info = specialized_interfaces[proto]                                                                                 
