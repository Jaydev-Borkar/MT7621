...
then
	...
	enable_internet=$(uci get nodogsplash.nodogsplash.enable_internet)
	gatewayinterface=$(uci get nodogsplash.nodogsplash.gatewayinterface)
	
	if [ "$enable_internet" = "0" ]
	then
		echo -n > /etc/dnsmasq.conf 
		
		local_IP=$(ifconfig $gatewayinterface | grep -i "inet addr" | awk '{print $2}' | cut -d':' -f2)
