...
PortSearch()
{
        ...
        then
                ...
                then
                        ...
                        SMSPortSymLink=$(find /dev -maxdepth 1 -name  CWAN1* | sort | sed -n 3"p")
                        # Get original/enumerated sms port
...
DeleteSMSConfiguration()
{
		...
		then 
			Devices="CWAN1_0,CWAN1_1"
        elif [ "$CellularOperationMode" = "dualcellularsinglesim" ]                                                          
		then
			Devices="CWAN1,CWAN2"
		else 
			Devices="CWAN1"
		fi
        ModifiedDevices=$(echo "$Devices" | grep -o $InterfaceName)
