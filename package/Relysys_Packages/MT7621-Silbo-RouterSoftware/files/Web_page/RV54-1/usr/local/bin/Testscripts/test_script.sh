...
#According to device name perform test.
...
if [ "$device_name" = "RV54-2" ] && [ "$wifi_enable_disable" = "0" ];then
	device_name="RV04-2"
elif [ "$device_name" = "RV54-1" ] && [ "$wifi_enable_disable" = "0" ];then
	device_name="RV04-1"
fi
...
external_usb="/usr/local/bin/Testscripts/Testresult/External_USB.txt"
wps="/usr/local/bin/Testscripts/Testresult/WPS.txt"
...
if [ "$device_name" = "RV54-2" ] || [ "$device_name" = "RV54-1" ] || [ "$device_name" = "RVW50" ]
then
	echo " "
...
at1_port()
{
	if [ "$device_name" = "RV54-2" ] || [ "$device_name" = "RV04-2" ]
	then
...
if [ "$device_name" = "RV54-2" ] || [ "$device_name" = "RV54-1" ] || [ "$device_name" = "RV04-2" ] || [ "$device_name" = "RV04-1" ]
then
	echo " "
	echo "========================================================================
					  Signal Strength LED Test
...
#Function to Perform External USB Test
external_USB_test()
{
echo " "
echo "=====================================================================================
...
echo " "
echo "====================Testing for USB3 device===================================="
echo " " 

for i in 1 2
do
	echo "Insert USB3 device and press enter"
	read  
	#Here, On the Gpio if test is fail...
	usb_gpio=$(cat /sys/class/gpio/gpio457/value) 
	if [ "$usb_gpio" = "1" ];then	
		echo 0 > /sys/class/gpio/gpio457/value 
		sleep 20
	fi
	
	USB3_device=$(cat /sys/bus/usb/devices/2-1/product)
	#when USB isn't found...
	if [ -z "$USB3_device" ]
	then
		echo "Make sure you have inserted the USB3 device properly. Rechecking in 5 seconds..."
		USB3_flag="0"
		sleep 5
	else
		USB3_flag="1"
...
if [ "$USB3_flag" = "0" ]
then
	echo -e "\e[1;31m [$time_stamp]	External USB3 Test = FAIL \e[0m" 
	echo "[$time_stamp]	External USB3 Test = FAIL" >> "$external_usb"
	res19=n
else
	echo -e "\e[1;32m [$time_stamp]	External USB3 Test = PASS \e[0m"
	echo "[$time_stamp]	External USB3 Test = PASS" >> "$external_usb"
	res19=y
fi

echo " "
echo "====================Testing for USB2 device===================================="
echo " " 

for i in 1 2
do
	echo "Insert USB2 device and press enter" 
	read
	sleep 8
	USB2_device=$(cat /sys/bus/usb/devices/1-1/product)
	echo $USB2_device
	#when USB isn't found...
	if [ -z "$USB2_device" ]
	then
		echo "Make sure you have inserted the USB2 device properly. Rechecking in 5 seconds..."
		USB2_flag="0"
		sleep 5
	else
		USB2_flag="1"
...
if [ "$USB2_flag" = "0" ]
then
	echo -e "\e[1;31m [$time_stamp]	External USB2 Test = FAIL \e[0m" 
	echo "[$time_stamp]	External USB2 Test = FAIL" >> "$external_usb"
	res20=n
else
	echo -e "\e[1;32m [$time_stamp]	External USB2 Test = PASS \e[0m"
	echo "[$time_stamp]	External USB2 Test = PASS" >> "$external_usb"
	res20=y
fi
...
if { [ "$USB3_flag" = "1" ] && [ "$USB2_flag" = "1" ]; } || { [ "$USB3_flag" = "1" ] && [ "$USB2_flag" = "0" ]; } || { [ "$USB3_flag" = "0" ] && [ "$USB2_flag" = "1" ]; }; then
    if [ -z "$USB3_device" ] && [ -z "$USB2_device" ]; then
 		echo -e "\e[1;32m [$time_stamp]	USB GPIO TEST = PASS \e[0m" 
 		echo "[$time_stamp]	USB GPIO TEST = PASS" >> "external_usb"
 		res21=y
    else
		echo -e "\e[1;31m [$time_stamp]	USB GPIO TEST = FAIL \e[0m" 
		echo "[$time_stamp]	USB GPIO TEST = FAIL" >> "$external_usb"
		res21=n
    fi
else
	echo -e "\e[1;31m [$time_stamp]	USB GPIO TEST = FAIL \e[0m" 
...
#Function to Perform Modem WPS Test
wps_test()
{
echo "========================WPS TEST======================================="

echo "Testing wps switch"
echo "Press and hold wps switch to observe the change in values"
 
for i in $(seq 1 10);
do
        gpio_val=$(cat /sys/class/gpio/gpio459/value)
        sleep 1
	echo $gpio_val
	if [ $gpio_val -eq 0 ] 
	then
...
if [ "$res22" = "y" ]
then
	echo -e "\e[1;32m [$time_stamp] WPS_Test=PASS \e[0m"
	echo "[$time_stamp] WPS_Test=PASS" >> $wps
else
	echo -e "\e[1;31m [$time_stamp] WPS_Test=FAIL \e[0m"
	echo "[$time_stamp] WPS_Test=FAIL" >> $wps
fi
echo " "
}

#perform all function according to device name...
if [ "$device_name" = "RV54-2" ]
then
	#Extra result already pass
	res14=y res19=y res20=y res21=y res22=y
...
then
	#Extra result already pass
	res4=y res5=y res6=y res13=y res19=y res20=y res21=y res22=y
...
then
	#Extra result already pass
	res1=y res2=y res3=y res4=y res5=y res6=y res12=y res13=y res14=y res16=y res18=y res22=y
...
then
	#Extra result already pass
	res1=y res2=y res3=y res4=y res5=y res6=y res12=y res13=y res14=y res16=y res18=y res19=y res20=y res21=y res22=y
...
# Copy all the Test File in final Report
	...
	cat $external_usb >> /usr/local/bin/Testscripts/Testresult/$filename.txt
	cat $wps >> /usr/local/bin/Testscripts/Testresult/$filename.txt
...
if [ "$res1" = "y" ] && [ "$res2" = "y" ] && [ "$res3" = "y" ] && [ "$res4" = "y" ] && [ "$res5" = "y" ] && [ "$res6" = "y" ] && [ "$res7" = "y" ] && [ "$res8" = "y" ] && [ "$res9" = "y" ] && [ "$res10" = "y" ] && [ "$res11" = "y" ] && [ "$res12" = "y" ] && [ "$res13" = "y" ] && [ "$res14" = "y" ] && [ "$res15" = "y" ] && [ "$res16" = "y" ] && [ "$res17" = "y" ] && [ "$res18" = "y" ] && [ "$res19" = "y" ] && [ "$res20" = "y" ] && [ "$res21" = "y" ] && [ "$res22" = "y" ]&& [ "$reboot_test" = "PASS" ];then
	echo "=============================================="
...
if [ "$wifi_enable_disable" = "1" ] || [ "$wifi_enable_disable" = "FF" ]; then
	...
	cp /usr/local/bin/Testscripts/original/without_wifi/network /etc/config/
	cp /usr/local/bin/Testscripts/original/without_wifi/dhcp /etc/config/
fi
...
uci set mwan3.CWAN1_0.enabled=1
uci set mwan3.CWAN1_1.enabled=1
uci commit mwan3
