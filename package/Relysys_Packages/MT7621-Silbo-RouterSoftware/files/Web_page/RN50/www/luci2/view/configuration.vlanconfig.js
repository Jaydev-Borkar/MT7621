L.ui.view.extend({
	...
	title: L.tr('VLAN'),
	description: L.tr('Do not mix tagged and untagged frames on the same physical interface. Please update after editing or deleting any changes and also update the network page accordingly.'),
	...
	tbCreateFormCallback: function () {
		...
		map.options.caption = L.tr('Tagged Port Configuration');
	...
	tbfCreateForm: function (mapwidget, tbfSectionID, tbfSectionType) {
		...
		var map = new mapwidget('taggedportconfig', {
			prepare: FormContent,
	...
	pbRenderContents: function (rv) {
		...
		for (var key in rv) {
			if (rv.hasOwnProperty(key)) {
				...
				list.row([VLANID, Port1, Port2, key]);
			}
		}
	...
	pbSectionAdd: function () {
		...
		var key;
		var SectionOptions = { vlanid: pbvlan, port1: pbport1, port2: pbport2 };
		if (status == "Add") {
			if (!id && !port1 && !port2) {
				...
				if(port1 || port2)
				{
					alert("Do not mix untagged and untagged frames on the same physical interface");
				}
			}

		}
		else {
			if ((key_2 == pbvlan || !id) && !port1 && !port2) {
	...
	tbSectionAdd: function () {
		...
		self.fCreateUCISection("taggedportconfig", "rule", SectionOptions).then(function (rv) {
			if (rv) {
				if (rv.section) {
					self.fCommitUCISection("taggedportconfig").then(function (res) {
						if (res != 0) {
							alert("Error: New Port Forward Configuration");
						}
						else {
	...
	tbSectionRemove: function (ev) {
		...
		self.deletetagvlanconfig('delete').then(function (rv) {
			self.fDeleteUCISection("taggedportconfig", "rule", tbSectionID).then(function (rv) {
				if (rv == 0) {
					self.fCommitUCISection("taggedportconfig").then(function (res) {
						if (res != 0) {
							alert("Error: Delete VLAN Configuration");
	...
	execute: function () {
		...
		this.fGetUCISections("taggedportconfig", "rule").then(function (rv) {
