...
#Product ID and Vendor ID for Modem2
usb_path_1_2=$(cat /sys/bus/usb/devices/usb1/1-2/product)

if [ -n "$usb_path_1_2" ];then
	Productid2=$(cat /sys/bus/usb/devices/usb1/1-2/idProduct)
	Vendorid2=$(cat /sys/bus/usb/devices/usb1/1-2/idVendor)	
else
	"Insert Modem2"	
fi
...
if [ "2c7c" = "$Vendorid2" ] && [ "6005" = "$Productid2" ];then
	echo "Modem2 is EC200A"
elif [ "2c7c" = "$Vendorid2" ] && [ "0125" = "$Productid2" ];then
	echo "Modem2 is EC25E"
elif [ "2c7c" = "$Vendorid2" ] && [ "030e" = "$Productid2" ];then
	echo "Modem1 is EM05G"
elif [ "2c7c" = "$Vendorid1" ] && [ "0306" = "$Productid1" ];then
	echo "Modem1 is EM06"
fi
...
then
	uci set system.system.model="silbo-RV04-2"
	uci set boardconfig.board.model="silbo-RV04-2"
	uci set boardconfig.board.boardname="silbo-RV04-2"
else
	uci set system.system.model="silbo-RV54-2"
	uci set boardconfig.board.model="silbo-RV54-2"
	uci set boardconfig.board.boardname="silbo-RV54-2"
fi
uci commit system
...
ret2=$(/root/usrRPC/script/Recycle_WAN2_PWR_Script.sh)
if [ $? -eq 0 ]; then
	echo "Modem2 powered off successfully"
else
	echo "Modem2 powered off failed"
fi
